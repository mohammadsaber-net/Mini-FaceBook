  ...
  return (
    <div className='w-full min-h-[calc(100vh-220px)] text-xs sm:text-base bg-white rounded-md p-1 text-slate-800'>
      ...
      <div className='flex flex-col bg-white'>
       {message.map((mes, ind) => {
        const otherUser = mes.from_user_id._id === userId
          ? mes.to_user_id 
          : mes.from_user_id;

        return (
          <Link to={`/messages/${otherUser._id}`} key={ind} className='flex group items-center p-1 hover:bg-blue-400 transition-all hover:text-white duration-200 mb-2 shadow'>
            <img src={otherUser.profile_picture || assets.sample_profile} className='w-8 h-8 rounded-full' alt="" />
            <div className='w-full ms-2'>
              <div className='flex justify-between'>
                <p className='font-medium overflow-hidden'>{otherUser.full_name}</p>
